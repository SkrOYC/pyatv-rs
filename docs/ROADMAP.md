...
Delivered on `feat/companion`. Live-verified against the Apple TV 4K (tvOS 27): pairing with an on-screen PIN, then app_list, device_info, features and power_state over the encrypted Companion session. Spec of record: `docs/research/companion-port-spec.md`. Landed: `pyatv-opack` (all pyatv vectors byte-exact, RISKS M8), Companion frame codec/connection/protocol/session bring-up, and `atvremote pair --protocol companion` — **verified live**: the Apple TV 4K on the LAN showed a PIN and produced credentials whose device key matches pyatv's own pairing of the same device. Live test credentials (not in the repo) are kept at `~/.pyatv-rs/live-rust.conf` (ours) and `~/.pyatv-rs/live-pyatv.conf` (pyatv format) for PIN-free autonomous validation of later phases.
